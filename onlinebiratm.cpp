...
void Model::set_model(Model* model) {
  memcpy(pi, model->pi, sizeof(int) * win_f);
...
void onlineModel::init(onlineModel* init_model) {
    if (init_model) {
    	for(int i=0; i<onlinewin_f; i++) onlinepi[i] = init_model->onlinepi[i];
    ...
    for(int i=0; i<onlinewin_f; i++) onlinepi[i] = util::random()*0.5 + 1;
    for (int k = 0; k < onlinenum_topics; k++) {
    	double total = 0;
...
Sentence ** convert_to_lda_corpus(senDocument ** batch_corpus, Model* model, int num_docs){
	...
	for(int d = 0; d<num_docs; d++){
		...
		lda_corpus[d] = new Sentence(words_ptr, words_cnt_ptr, word_num_inSen, num_topics, win_f);
...
void begin_ratm(Configuration config, char* inputfile, char* model_root, onlineModel* onlinemodel,
    ...
    //model for train set
    Model* model = new Model(batch_num_docs, 0, batch_num_docs, 0, batch_num_words, total_num_words, num_topics, win);
    Model* oldmodel = new Model(batch_num_docs, 0, batch_num_docs, 0, batch_num_words, total_num_words, num_topics, win);
